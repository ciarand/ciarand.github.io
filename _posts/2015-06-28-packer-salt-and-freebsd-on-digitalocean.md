...
actual nodes at runtime), it only supports running Salt in a "masterless"
state. That's perfect for our needs.
...
(+ a timestamp), and then shut the Droplet down. If something failed, now would
be a really good time to try and debug it. You can increase the verbosity of
what Packer prints to stdout by setting the environment variable `PACKER_LOG`
to 1, like so: `PACKER_LOG=1 packer build build.json`.
...
environment variables, so we have to adjust the template it uses. In this case,
that means adding the `env` command before the ``{{.Vars}}`` part.
...
user. Specifically, it uses the `user.present` API. Note that the only place we
specify the username (`msmith`) is in the name of the hash. This was confusing
to me at first, but it greatly reduces some of the boilerplate repetition.
...
It's also important to note that this server isn't production ready (the
`sshd_config` file hasn't been changed, for example). That being said, taking
the next steps should be fairly simple. I encourage you to read through the
[Salt][] documentation as you move forward.
